...
class ModelHandler_dcrnn(object):
    def __init__(self, config, train_loader, val_loader, test_loader, adj0, dist):
        ...
        """
        self.model = IDGL(config,self.adj1)
    ...
    def run_whole_epoch(self,data_loader,task_type,verbose=None,out_predictions=False):
        ...
        for batch_idx,data in enumerate(data_loader):
            ...
            # 重新初始化模型并进行读取数值
            if load_modelParam:
                modelA = IDGL(self.config, self.adj1)
            ...
            x, y = self._prepare_data(t2m_hist, t2m_label)
            t2m_pred = self.model.encoder(x,y)
            t2m_pred = t2m_pred.permute(1, 0, 2)
            t2m_pred = torch.unsqueeze(t2m_pred,dim=-1)
    ...
    def _get_x_y_in_correct_dims(self, x, y):
        ...
        """
        batch_size = x.size(1)
        x = x.view(int(self.config['train']['hist_len']), batch_size, int(self.config['dcrnn']['num_nodes']) * int(self.config['dcrnn']['input_dim']))
        y = y[..., :int(self.config['dcrnn']['output_dim'])].view(int(self.config['train']['pred_len']), batch_size,
                                          int(self.config['dcrnn']['num_nodes']) * int(self.config['dcrnn']['output_dim']))
        return x, y