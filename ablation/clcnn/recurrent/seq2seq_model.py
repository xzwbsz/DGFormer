...
class CLCRNModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, loc_info, sparse_idx, geodesic, angle_ratio, config, logger=None):
        '''
        ...
        self.encoder_model = EncoderModel(sparse_idx, geodesic, angle_ratio, self.conv_ker, config)
        self.decoder_model = DecoderModel(sparse_idx, geodesic, angle_ratio, self.conv_ker, config)
        self.cl_decay_steps = int(config['clcrn']['cl_decay_steps'])
        self.use_curriculum_learning = bool(config['clcrn']['use_curriculum_learning'])
        self._logger = logger
