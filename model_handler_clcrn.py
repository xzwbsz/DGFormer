...
class ModelHandler_clcrn(object):
    def __init__(self, config, train_loader, val_loader, test_loader, adj0, dist, lonlat=None):
        self.config = config
        """
            1 创建评价指标 并指定数据结构
        """
        ...
        """
        self.model = IDGL(config, self.adj1, lonlat=lonlat)
    ...
    def run_whole_epoch(self, data_loader, task_type, verbose=None, out_predictions=False):
        ...
        if task_type == "train":
            ...
            if os.path.exists("./save_train_param/Param0.pth"):
                ...
                self.model.load_state_dict(load_data, False)
        else:
            self.model.eval()
        ...
        for batch_idx, data in enumerate(data_loader):
            ...
            # 重新初始化模型并进行读取数值
            if load_modelParam:
                modelA = IDGL(self.config, self.adj1, lonlat=self.lonlat)
                self.model = modelA.to(self.device)
                load_data = torch.load(pth_idx_str) if task_type == "train" else torch.load(
                    "./save_train_param/Param0.pth")
                self.model.load_state_dict(load_data, False)
                pth_idx_str = "./save_train_param/Param"
                loss = torch.tensor(0)
                load_modelParam = False
            ...
            """111"""
            t2m_hist = t2m_hist.permute(1, 0, 2, 3)
            t2m_label = t2m_label.permute(1, 0, 2, 3)
            t2m_pred = self.model.encoder(t2m_hist, t2m_label)
            ...
            if task_type == "test":
                t2m_hist = t2m_hist.permute(1, 0, 2, 3)
                t2m_pred = t2m_pred.permute(1, 0, 2, 3)
                t2m_pred_val = np.concatenate([t2m_hist.cpu().detach().numpy(), t2m_pred.cpu().detach().numpy()],
                                              axis=1) * self.t2m_std + self.t2m_mean
    ...
    def add_graph_loss(self, out_adj, features):  # 输入 A(t) 和 features:X
        ...
        graph_loss += self.config['idgl']['smoothness_ratio'] * torch.trace(
            torch.mm(features.transpose(-1, -2), torch.mm(L, features))) / int(
            np.prod(out_adj.shape))  # Ω(A,X) = 1/n^2 tr(X^T L X) 最小化平滑损失 | tr() 表示对角线元素总和
        ones_vec = to_cuda(torch.ones(out_adj.size(-1)), self.device)
        graph_loss += -self.config['idgl']['degree_ratio'] * torch.mm(ones_vec.unsqueeze(0), torch.log(
            torch.mm(out_adj, ones_vec.unsqueeze(-1)) + VERY_SMALL_NUMBER)).squeeze() / out_adj.shape[
                          -1]  # -β/n 1^T log(A1)
        graph_loss += self.config['idgl']['sparsity_ratio'] * torch.sum(torch.pow(out_adj, 2)) / int(
            np.prod(out_adj.shape))  # f(A) = ↑ + γ/n^2 ||A||_2_F
