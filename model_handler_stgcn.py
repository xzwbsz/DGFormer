...
from lib_stgcn import utility

class ModelHandler_stgcn(object):
    def __init__(self,config,train_loader,val_loader,test_loader,adj0,dist):
        ...
        """
        self.gso = self.get_gso()
        self.model = IDGL(config,self.adj0,gso=self.gso)
    ...
    def run_whole_epoch(self,data_loader,task_type,verbose=None,out_predictions=False):
        ...
        for batch_idx,data in enumerate(data_loader):
            ...
            # 重新初始化模型并进行读取数值
            if load_modelParam:
                modelA = IDGL(self.config, self.adj1,self.gso)
            ...
            t2m_hist = t2m_hist.permute(0, 3, 1, 2)

            t2m_pred = self.model.encoder(t2m_hist)
            t2m_pred = t2m_pred.permute(0, 1, 3, 2)
            ...
            if task_type == "test":
                t2m_hist = t2m_hist.permute(0, 2, 3, 1)
    ...
    def get_gso(self):
        ...
        adj = self.adj0
        n_vertex = int(self.config['stgcn']['n_vertex'])
        gso = utility.calc_gso(adj, self.config['stgcn']['gso_type'])
        if self.config['stgcn']['graph_conv_type'] == 'cheb_graph_conv':
            gso = utility.calc_chebynet_gso(gso)
        gso = gso.toarray()
        gso = gso.astype(dtype=np.float32)
        gso = torch.from_numpy(gso).to(self.device)
        return gso